...
class App extends Component {
  ...
  onTextCallback(text) {
    ...
    let sortByData;
    var self = this;
    sortByData = this.state.content.split(' ');
    let clearArrayTable = $('#example').DataTable();
    let finalCheck = this.state.content.split('short ');
    if (finalCheck.length > 1) {
      $("#example thead tr th").each(function (index) {

        if (sortByData[0].toLowerCase() === 'short') {
    ...
    if (!this.finalCheckFlag) {
      self.clearTable = $('#example').DataTable();
      $('#example').focus();
      $('#example').on('focus', function () {
        self.clearTable.search(text).draw();
      });
    }
  }
  ...
  render() {
    return (
      <div className="App">
        ...
        <div>
          <WebSpeech lang={this.state.lang} onText={this.onTextCallback} />
        </div>
        <br></br>
          ...
          {/* <Tbl data={this.dataSet} voicedata={this.state.content}>
